...
<script>
  ...
  export default {
    created() {
      //   let artistParam = this.$route.params.id;
      ...
      firebase.db
        ...
        .get()
        .then(snapshot => {
          const {file_name, image_url, desc, name, khName} = snapshot.data();
          this.previousDoc = snapshot.data();
          this.fileList = [{
            name: file_name,
            url: image_url
          }]
          ...
          if (valid) {
            ...
            firebase.db
              ...
              .then(snapshot => {
                ...
                this.fileList.map(o => {
                  if (o.name !== vm.previousDoc.file_name) {
